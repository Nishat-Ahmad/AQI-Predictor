...
export function renderHeroAtmosphere(current, hourly) {
    ...
    if (currentAqiValEl) currentAqiValEl.innerText = Number(current.consensus_aqi).toFixed(2);
    ...
    if (conditionSummaryEl) conditionSummaryEl.innerText = `Atmospheric Summary: ${getAtmosphericHeadline(current.consensus_aqi, current.pm2_5)}`;
    if (currentTimestampEl) currentTimestampEl.innerText = `Forecast: ${current.datetime}`;
    if (currentAqiAdvisoryEl) currentAqiAdvisoryEl.innerText = getHealthAdvisory(current.consensus_aqi);
...
export function renderModelTiles(current) {
    ...
    if (rfScoreEl) rfScoreEl.innerText = current.rf_aqi ? `${Number(current.rf_aqi).toFixed(2)}` : "--";
    if (ridgeScoreEl) ridgeScoreEl.innerText = current.ridge_aqi ? `${Number(current.ridge_aqi).toFixed(2)}` : "--";
    if (dlScoreEl) dlScoreEl.innerText = current.dl_aqi ? `${Number(current.dl_aqi).toFixed(2)}` : "--";
    if (consensusScoreEl) consensusScoreEl.innerText = `${Number(current.consensus_aqi).toFixed(2)} / 5.0`;
}
